...
def batch_np_matrix_to_pycolmap_adaptive_thre(
    points3d,
    extrinsics,
    intrinsics,
    tracks,
    ...
    if max_reproj_error is not None:
        ...
        cur_reproj_error = init_reproj_error
        reproj_mask = np.zeros_like(masks) * 1.
        fill_mask = np.zeros_like(masks[0]) * 1.
        while cur_reproj_error <= max_reproj_error:
            cur_reproj_mask = projected_diff < cur_reproj_error
            cur_masks = np.logical_and(masks, cur_reproj_mask) * 1.  # satisfy reproj error and visibility
            cur_inlier_num = cur_masks.sum(0)
            cur_valid_mask = (cur_inlier_num >= 2) * 1.
            update_mask = 1. - (fill_mask > 0)
            # cur_update_num = np.sum(update_mask)
            reproj_mask += update_mask * cur_valid_mask * cur_masks
            fill_mask += update_mask * cur_valid_mask
...
def batch_np_matrix_to_pycolmap(
    ...
    if masks.sum(1).min() < min_inlier_per_frame:
        view_inlier_num = masks.sum(1)
        print(view_inlier_num)
        view_mask = view_inlier_num >= min_inlier_per_frame
        masks = masks[view_mask]
        extrinsics = extrinsics[view_mask]
        intrinsics = intrinsics[view_mask]
        tracks = tracks[view_mask]
        print("drop view before BA:", np.arange(N)[~view_mask])
        N = tracks.shape[0]
        # print(f"Not enough inliers per frame, skip BA.")
        # return None, None
    ...
    if return_reproj_mask:
        return reconstruction, valid_mask, reproj_mask
    return reconstruction, valid_mask
